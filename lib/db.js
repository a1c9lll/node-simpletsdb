...
SimpleTSDB.prototype.metricExists = function (options) {
  ...
  return axios({
    method: 'GET',
    url: `http://${this.host}:${this.port}/metric_exists?metric=${options.metric}`
  })
    .then(({ data }) => data)
    .catch(err => {
...
SimpleTSDB.prototype.queryPoints = function (query) {
  ...
  if (Object.hasOwnProperty.call(query, 'tags') && !Array.isArray(query.tags)) {
    throw TypeError('query.tags must be array of strings');
  }
